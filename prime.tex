...
%\emph{namely the notion of Containment Division Ring (CDR)}. 
...
$ \cdot_{\iota}$; and the relations $\subseteq$ and $\Id{isideal}$.
...
%we can choose any axiomatization of them, since for the (partial) blend we just take into account only 
...
%
%\[e\dannydiv g:=g|e.\]
$e\dannydiv g:=g|e$,  i.e.\ there exists an integer $c$ such that $e=c * g$.
...
%blend.  
Let us define a unary relation $\Id{isprime}$ on $\mathbb{Z}$ as
follows: for all $p\in \mathbb{Z}$, $\Id{isprime}(p)$ holds if $p\neq 1$
...
\[ \Id{Prime}=\left\{ p\in \mathbb{Z}: \Id{isprime}(p)\right\}\]
% Now, it is an
...
operation, $\Id{prime}$ as a predicate and $\dannydiv$ as a binary
...
any element $P \in G$
(i.e., an ideal of $S$) satisfies the predicate $\Id{isprime}$ if and only
if
\begin{align*}
...
% b\in P)).\]
Thus, the predicate $\Id{isprime}$ turns out to be the
...
set of ideals of $S$, $\Id{isprime}$ is the predicate specifying primality
for ideals of $S$ and $\Id{Prime}$ is the collection of all prime ideals of
$S$.
...
(\Id{S}), the set of all its ideals (\Id{Generic}), the set all its
prime ideals (\Id{SimplePrime}) and a primality predicate
(\Id{IsPrime}).  We present in Listing~\ref{listing:prime_colim} just
the theory corresponding to the colimit (omitting details of ring
