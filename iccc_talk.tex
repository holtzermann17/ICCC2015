...
\jc{Sure.  OK, so, the paper is called ``Computational Poetry
  ...
  more broadly applicable than CC, and it \emph{asks: ``How a created
    artefact can tell us about its own making?''}  The paper takes
  this idea and runs with it a bit, to suggest that in principle
  computers can engage in dialogue about poems.  Out of this comes the
  ...
  thought experiment in which an improved version of the FloWr system
  that's being developed in the CCG group at Goldsmiths could be used
  ...
  core idea of the workshop approach. \emph{The paper is claiming that
...
\direct{The projection switches to a blank screen.  \refer{Joe} takes
  notes on what people are saying, and these notes begin to accumulate
  on the screen.  If possible, the notes should resemble some sort of
  poem by the end of this scene.}
...
\aj{Well, first of all, I think this paper makes a reasonable start
  ...
  things after they've created but for helping create them in the
...
\aj{Following on from what Teresa said, my only concern here is that
  too little has been done in terms of modeling and system-building for
  others to effectively build upon, or argue for or against.}

\cg{I think the idea of the workshop and of a process-based theory of
  poetics is really intriguing.  But the current elaboration is quite
  far from discussing the actual computational characteristics the
  agents should have to make this work.  I think the paper is really
  more at the level of a discussion than a thought experiment.  I find
  it a bit hard to see the computational focus, although maybe that's
  because you're trying to make the paper do double duty as a kind of
  manifesto.  But, don't get me wrong, I think it is a beginning of a
  very interesting challenge and food for thought for later discussion
  and more precise elaboration.  Oh, and there are also a few typos
  should be corrected, I can pass you a list that I found.  Any other
  comments?}
...
\cg{I think there's a lot there, but it would be good if the work was
  presented with some more structure.  If not in the paper itself,
  then at the conference: a well structured and concrete presentation,
  perhaps following an example that looks into the role of different
  agents could help spark the discussion.  \direct{Breaking the 4th
    wall.} With that, let's take some questions and comments from the
  audience.}
